...
export const run: RouteRun = async (app, req: GetRequest, res) => {
	...
	if (!isNaN(req.query.id)) data = await announcements.findOne({ id: req.query.id });
	else if (req.query.latest) data = await announcements.findOne();
	else data = await announcements.find().limit(5).sort({ $natural: -1 }).toArray();
